...
const ImprovedDonut = ({ data, width }) => {
  // improved donut chart todo
  // should have option to display chart data in a table
  // tooltip

  const [activeIndex, setActiveIndex] = useState(undefined)
  const [itemSelected, setItemSelected] = useState(undefined)
  const [fieldLabel, setFieldLabel] = useState(undefined)
    ...
    const labelTimeout = () => {
      ...
      const renderLabel = (state, params) => {
          const {
              cx,
              cy,
              midAngle,
      ...
      const renderActiveLabel = (state, params) => {
